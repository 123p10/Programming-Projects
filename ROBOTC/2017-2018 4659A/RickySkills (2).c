...
#pragma config(Motor,  port2,           DriveL3,       tmotorVex393_MC29, openLoop)
...
#pragma config(Motor,  port7,           DriveR1,       tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port8,           DriveR2,       tmotorVex393_MC29, openLoop)
#pragma config(Motor,  port9,           DriveR3,       tmotorVex393_MC29, openLoop, reversed)
...
void auton()
{
	...
	while(!moveDriveAsync(inchesToDistance(-30), 0.3));
	wait1Msec(300);
	while(!turnDriveAsync(degreesToRotation(-90)));

	//while(!turnDriveAsync(degreesToRotation(45)));
	while(!moveDriveAsync(inchesToDistance(3), kPMogo));
	ClearTimer(T1);
	while(!turnDriveAsync(degreesToRotation(-140))){if(time1[T1] > 3000) break;}
	wait1Msec(300);
	ClearTimer(T1);
	while(!moveDriveAsync(inchesToDistance(40), kPMogo)){if(time1[T1] > 1000) break;}
//	while(!moveMogoAsync(mogoExtended, 3));
//	while(!moveDriveAsync(inchesToDistance(-20), kPNoMogo));
	ClearTimer(T1);
	while(!moveMogoAsync(mogoExtended, 3)){if(time1[T1] > 1000) break;}
//	while(!moveMogoAsync(mogoRetracted, 3));

	while(!moveDriveAsync(inchesToDistance(-20), kPNoMogo));
	//MOGO in 20 pt zone
/*
	while(!moveDriveAsync(inchesToDistance(-5), kPMogo+1));
	while(!turnDriveAsync(degreesToRotation(-160)));
	while(!moveDriveAsync(inchesToDistance(10), kPNoMogo));
	while(!turnDriveAsync(degreesToRotation(-30)));
	//MOGO in hold

	while(!moveMogoAsync(mogoExtended, 3));
	while(!moveDriveAsync(inchesToDistance(15), kPNoMogo));
	while(!moveMogoAsync(mogoRetracted, 3));
	//MOGO picked up

	while(!turnDriveAsync(degreesToRotation(180)));
	while(!moveDriveAsync(inchesToDistance(23), kPMogo));
	while(!moveMogoAsync(mogoExtended, 3));
	//MOGO placed in 10 pt zone

	while(!moveDriveAsync(inchesToDistance(6), kPMogo));
	while(!moveDriveAsync(inchesToDistance(-3), kPMogo));
	while(!moveMogoAsync(mogoRetracted, 3));

	/*wait1Msec(1000);
	while(!moveDriveAsync(inchesToDistance(-18), kPMogo));
	while(!moveMogoAsync(mogoRetracted, 3));
	wait1Msec(1000);
	while(!turnDriveAsync(degreesToRotation(90)));

	while(!moveDriveAsync(inchesToDistance(12), kPNoMogo));
	while(!turnDriveAsync(degreesToRotation(90)));
	while(!moveMogoAsync(mogoExtended, 3));
	while(!moveDriveAsync(inchesToDistance(12), kPNoMogo));
...
*/
	...
	while(!moveDriveAsync(inchesToDistance(-12), kPNoMogo)) { moveMogoAsync(mogoRetracted, 3); }

	while(!turnDriveAsync(degreesToRotation(-220)));
	while(!moveDriveAsync(inchesToDistance(5), kPNoMogo));
	while(!turnDriveAsync(degreesToRotation(55)));
	while(!moveMogoAsync(mogoExtended, 3));
	while(!moveDriveAsync(inchesToDistance(12), kPNoMogo));
	while(!moveMogoAsync(mogoRetracted, 3));
	while(!moveDriveAsync(inchesToDistance(-10), kPMogo));
	while(!turnDriveAsync(degreesToRotation(80)));
	while(!moveDriveAsync(inchesToDistance(36), kpNoMogo));
	...
	while(!turnDriveAsync(degreesToRotation(-100)));
	...
	while(!moveDriveAsync(inchesToDistance(20), kPMogo));
	while(!moveMogoAsync(mogoExtended, 3));*/
}
